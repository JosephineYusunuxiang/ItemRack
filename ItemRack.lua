...
ItemRack.SlotInfo = {
	...
	[1] = { name="HeadSlot", real="头盔", INVTYPE_HEAD=1 },
	[2] = { name="NeckSlot", real="项链", INVTYPE_NECK=1 },
	[3] = { name="ShoulderSlot", real="肩膀", INVTYPE_SHOULDER=1 },
	[4] = { name="ShirtSlot", real="衬衣", INVTYPE_BODY=1 },
	[5] = { name="ChestSlot", real="胸甲", INVTYPE_CHEST=1, INVTYPE_ROBE=1 },
	[6] = { name="WaistSlot", real="腰带", INVTYPE_WAIST=1 },
	[7] = { name="LegsSlot", real="裤子", INVTYPE_LEGS=1 },
	[8] = { name="FeetSlot", real="鞋", INVTYPE_FEET=1 },
	[9] = { name="WristSlot", real="护腕", INVTYPE_WRIST=1 },
	[10] = { name="HandsSlot", real="手套", INVTYPE_HAND=1 },
	[11] = { name="Finger0Slot", real="戒指1", INVTYPE_FINGER=1, other=12 },
	[12] = { name="Finger1Slot", real="戒指2", INVTYPE_FINGER=1, other=11 },
	[13] = { name="Trinket0Slot", real="饰品1", INVTYPE_TRINKET=1, other=14 },
	[14] = { name="Trinket1Slot", real="饰品2", INVTYPE_TRINKET=1, other=13 },
	[15] = { name="BackSlot", real="披风", INVTYPE_CLOAK=1 },
	...
	[19] = { name="TabardSlot", real="战袍", INVTYPE_TABARD=1 },
}
...
ItemRack.TooltipInfo = {
	...
	{"ItemRackOptSetsHideCheckButton","隐藏","在弹出的套装菜单中隐藏此套装。"},
...
do
	...
	function ItemRack.ListSetsHavingItem(tooltip, id)
		...
		for name in pairs(data) do
			tooltip:AddDoubleLine("ItemRack套装: ", name, 0,.6,1, 0,.6,1)
			data[name] = nil
		end
		tooltip:Show()
	end
			...
			if ItemRack.GetCountByID(item)==0 then
				...
				if bankBag then
					...
					if freeBag and not SpellIsTargeting() and not GetCursorInfo() then
						...
						ItemRack.Print("背包里没有足够的空间从银行里取出这件物品。")
					end
				end
			else
				...
				if bankBag then
					...
					ItemRack.Print("银行里没有足够的空间存放这件物品。")
				end
			end
		else
...
function ItemRack.MinimapOnEnter(tooltip)
	...
	tooltip:AddLine("ItemRack自动换装插件")
	tooltip:AddLine("左键: 穿上一套装备",.8,.8,.8,1)
	tooltip:AddLine("右键: 打开插件设置",.8,.8,.8,1)
	tooltip:AddLine("Alt 左键: 显示隐藏套装",.8,.8,.8,1)
	tooltip:AddLine("Alt 右键: 切换事件",.8,.8,.8,1)
	tooltip:AddLine("Shift 左键: 取消装备",.8,.8,.8,1)
	tooltip:AddLine("玉素奴香@哈霍兰CN汉化了IR3.75版",.8,.8,.8,1)
end
...
function ItemRack.SetSetBindings()
	...
	if not inLockdown then
		...
		ItemRack.Print("无法在战斗中保存热键，请在脱战后重试！")
	end
end
...
function ItemRack.SlashHandler(arg1)
	...
	if arg1 and string.match(arg1,"equip") then
		...
		if not set then
			ItemRack.Print("用法：/itemrack equip 装备套装名称")
			ItemRack.Print("举例: “/itemrack equip PVP套装”")
		else
	...
	elseif arg1 and string.match(arg1,"toggle") then
		...
		if not sets then
			ItemRack.Print("用法: /itemrack toggle 第一套名称[, 第二套名称]")
			ItemRack.Print("举例: “/itemrack toggle PVP套装, 拉怪套装”")
		else
	...
	elseif arg1=="opt" or arg1=="options" or arg1=="config" then
		...
		ItemRack.Print("/itemrack opt : 打开ItemRack设置窗口。")
		ItemRack.Print("/itemrack equip set name : 装备名为'set name'的套装。")
		ItemRack.Print("/itemrack toggle set name[, second set] : 切换名为'set name'的套装 。")
		ItemRack.Print("/itemrack reset : 重置按键绑定。")
		ItemRack.Print("/itemrack reset everything :将ItemRack所有设置重置为默认值。")
	...
	for _,i in pairs(ItemRackUser.Sets[setname].equip) do
		...
		if bag then
			...
			if freeBag then
				...
				ItemRack.Print("背包没有足够的空间来放置来自'"..setname.."'套装的物品。")
				return
			end
		end
	end
	...
	for _,i in pairs(ItemRackUser.Sets[setname].equip) do
		if i~=0 then
			...
			if freeBag then
				...
				ItemRack.Print("银行没有足够的空间来存放来自'"..setname.."'套装的物品。")
				return
			end
		end
	end
...
function ItemRack.ResetEverything()
	StaticPopupDialogs["ItemRackCONFIRMRESET"] = {
		text = "这将重置ItemRack恢复到默认状态，擦除所有套装、按钮、事件和设置。\n界面将重新加载。要继续吗？",
