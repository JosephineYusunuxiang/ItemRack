...
ItemRack.CheckButtonLabels = {
	...
	["ItemRackOptQueueEnableText"] = "此栏位自动执行队列",
	...
	["ItemRackOptEventEditBuffUnequipText"] = "buff结束时还原装备",
	["ItemRackOptEventEditBuffNotInPVPText"] = "PVP状态除外",
	["ItemRackOptEventEditBuffNotInPVEText"] = "PVE状态除外",
	["ItemRackOptEventEditStanceUnequipText"] = "切离姿态时还原装备",
	["ItemRackOptEventEditZoneUnequipText"] = "离开区域时还原装备",
...
function ItemRackOpt.OnLoad(self)
	...
	ItemRackOpt.OptInfo = {
		...
		{type="check",optset=ItemRackUser,variable="Locked",label="锁定按钮",tooltip="锁定按钮和选择菜单。"},
		{type="check",optset=ItemRackUser,variable="EnableEvents",label="启用事件",tooltip="启用事件自动换装。"},
		{type="check",optset=ItemRackUser,variable="EnableQueues",label="启用自动队列",tooltip="启用自动队列自动换装。"},
		{type="check",optset=ItemRackUser,variable="EnablePerSetQueues",depend="EnableQueues",label="启用套装自动队列",tooltip="启用套装自动队列更换"},
		{type="number",optset=ItemRackUser,variable="ButtonSpacing",button=ItemRackOptButtonSpacing,label="按钮间距",tooltip="按钮之间的填充距离。",combatlock=1},
		{type="slider",button=ItemRackOptButtonSpacingSlider,variable="ButtonSpacing",label="按钮间距",tooltip="按钮之间的填充距离。", min=0, max=24, step=1, form="%d",combatlock=1},
		{type="number",optset=ItemRackUser,variable="Alpha",button=ItemRackOptAlpha,label="透明度",tooltip="按钮和菜单的透明度（alpha）。"},
		{type="slider",button=ItemRackOptAlphaSlider,variable="Alpha",label="透明度",tooltip="按钮和菜单的透明度（alpha）。", min=.1, max=1, step=.05, form="%.2f"},
		{type="number",optset=ItemRackUser,variable="MainScale",button=ItemRackOptMainScale,label="按钮缩放",tooltip="缩放按钮的大小。",combatlock=1},
		{type="slider",button=ItemRackOptMainScaleSlider,variable="MainScale",label="按钮缩放",tooltip="缩放按钮的大小。", min=.5, max=2, step=.05, form="%.2f",combatlock=1},

		{type="number",optset=ItemRackUser,variable="MenuScale",button=ItemRackOptMenuScale,label="菜单缩放",tooltip="缩放依附按钮菜单的尺寸。"},
		{type="slider",button=ItemRackOptMenuScaleSlider,variable="MenuScale",label="菜单缩放",tooltip="缩放依附按钮菜单的尺寸。", min=.5, max=2, step=.05, form="%.2f"},

		{type="check",optset=ItemRackUser,variable="SetMenuWrap",label="设置菜单换行按钮数目",tooltip="设置每一行的长度，如果不选，则让ItemRack自行决定。"},
		...
		{type="check",optset=ItemRackSettings,variable="MenuOnShift",label="Shift打开菜单",tooltip="只在按住Shift键时显示依附菜单。"},
		{type="check",optset=ItemRackSettings,variable="MenuOnRight",label="右键打开菜单",tooltip="通过右键按钮打开依附菜单。",combatlock=1},
		{type="check",optset=ItemRackSettings,variable="HideOOC",label="脱战时隐藏",tooltip="脱战时隐藏按钮。",combatlock=1},
		{type="check",optset=ItemRackSettings,variable="HidePetBattle",label="宠物对战时隐藏",tooltip="宠物对战时隐藏按钮。"},
		{type="check",optset=ItemRackSettings,variable="Notify",label="物品可用时提醒",tooltip="物品冷却时间结束时发出提示。"},
		{type="check",optset=ItemRackSettings,variable="NotifyThirty",label="30s时提醒",tooltip="物品还剩30秒冷却时提示。"},
		{type="check",optset=ItemRackSettings,variable="NotifyChatAlso",label="聊天框提醒",tooltip="同时向聊天框发送冷却通知。"},
		{type="check",optset=ItemRackSettings,variable="ShowSetInTooltip",label="鼠标提示显示",tooltip="在鼠标提示窗口显示物品被应用于哪些套装中。"},
		{type="check",optset=ItemRackSettings,variable="ShowTooltips",label="显示提示",tooltip="显示提示，就像你正看到的一样。"},
		{type="check",optset=ItemRackSettings,variable="TinyTooltips",depend="ShowTooltips",label="精简提示",tooltip="精简提示，仅显示物品名称、冷却时间和耐久。"},
		{type="check",optset=ItemRackSettings,variable="TooltipFollow",depend="ShowTooltips",label="提示跟随鼠标",tooltip="在鼠标位置显示提示。"},
		{type="check",optset=ItemRackSettings,variable="CooldownCount",label="冷却计数",tooltip="在装备上显示冷却数字。"},
		{type="check",optset=ItemRackSettings,variable="LargeNumbers",depend="CooldownCount",label="更大字号",tooltip="冷却计数使用更大号的字体。"},
		...
		{type="check",optset=ItemRackSettings,variable="AllowEmpty",label="允许空栏",tooltip="允许在套装中添加空栏位。"},
		{type="check",optset=ItemRackSettings,variable="AllowHidden",label="允许隐藏装备",tooltip="按住Alt+点击后隐藏或显示装备，按住Alt显示所有装备。"},
		{type="check",optset=ItemRackSettings,variable="HideTradables",label="隐藏可交易物品",tooltip="不显示尚未绑定的装备。"},
		{type="check",optset=ItemRackSettings,variable="ShowMinimap",label="显示小地图按钮",tooltip="在小地图上显示快速换装按钮。"},
		{type="check",optset=ItemRackSettings,variable="MinimapTooltip",depend="ShowMinimap",label="显示小地图提示",tooltip="启用后，鼠标经过小地图按钮时显示提示。"},
		{type="check",optset=ItemRackSettings,variable="TrinketMenuMode",label="饰品菜单模式",tooltip="鼠标经过任意饰品格时，在上饰品栏旁弹出饰品菜单，左键装备至上饰品栏，右键装备至下饰品栏。"},
		{type="check",optset=ItemRackSettings,variable="AnchorOther",depend="TrinketMenuMode",label="锚点另一个饰品",tooltip="饰品菜单依附在下饰品栏旁。"},
		{type="check",optset=ItemRackSettings,variable="EquipToggle",label="套装切换",tooltip="当一个套装已经被装备时，再次选择该套装将解除该套装。"},
		{type="check",optset=ItemRackSettings,variable="ShowHotKeys",label="显示绑定按键",tooltip="在按钮上显示绑定的热键"},
		{type="check",optset=ItemRackSettings,variable="EquipOnSetPick",label="装备选项中的物品",tooltip="选中启用快速套装，可以直接在设置页面或通过拖动快速修改套装。"},
		{type="check",optset=ItemRackSettings,variable="CharacterSheetMenus",label="角色界面装备菜单",tooltip="选增强人物界面，在每个装备栏旁显示可替换装备。"},
		{type="check",optset=ItemRackSettings,variable="DisableAltClick",label="禁用Alt点击",tooltip="禁用Alt+点击装备栏启用顺序装备。（当你设置了Alt自我施法时请勾选） ",combatlock=1},
		{type="label",label=""},
		{type="button",button=ItemRackOptKeyBindings,label="栏位绑定热键",tooltip="为一个装备栏设置一个快捷键。",combatlock=1},
		{type="button",button=ItemRackOptResetBar,label="重置按钮",tooltip="移除所有按钮并恢复默认透明度和缩放设置。",combatlock=1},
		{type="button",button=ItemRackOptResetEvents,label="重置事件",tooltip="删除所有自定义事件，并还原默认值。"},
		{type="button",button=ItemRackOptResetEverything,label="重置全部",tooltip="擦除所有设置、集合和事件，将插件恢复到默认状态。",combatlock=1},
	}
...
function ItemRackOpt.BindFrameOnShow()
	...
	ItemRackOpt.Binding.currentKey=GetBindingKey("CLICK "..ItemRackOpt.Binding.buttonName..":LeftButton") or "无绑定按键"
	...
	for i=1,11 do
		...
		if sortList and idx<=#(sortList) then
			if sortList[idx]==0 then
				name,texture,quality = "-- 在此处停止队列 --","Interface\\Buttons\\UI-GroupLoot-Pass-Up",1
			else
...
function ItemRackOpt.SortListOnEnter(self)
	...
	if list[idx] then
		if list[idx]==0 then
			ItemRack.OnTooltip(self,"在此处停止队列","移动此标志到预期的队列末尾。例如，如果你有一个带有被动效果的主动饰品，并且如果没有更好的饰品停止冷却来替换，而你想使用这个被动效果时，可以将它放在队尾。")
		else
...
function ItemRackOpt.EventListOnEnter(self,child)
	...
	if eventType=="Buff" then
		if event.Anymount then
			...
			desc = desc.."获得BUFF时。"..event.Buff
		end
	elseif eventType=="Stance" then
		if event.Stance == 0 then
			desc = desc.."取消变形。"
		else
			desc = desc.."进入姿态/形态:"..event.Stance.."时。"
		end
	elseif eventType=="Zone" then
		...
		desc = "|cFFBBBBBB触发脚本 "..event.Trigger
...
function ItemRackOpt.EventEditPopulateFrame()
	...
	if idx and event then
		...
		if event.Anymount then
			...
			ItemRackOptEventEditBuffName:SetText("任意坐骑")
		end
